...
function parseSettingsCSV(csv) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        } else if (setting === 'MaintenanceMode') {
            localStorage.setItem('maintenanceMode', value.toLowerCase() === 'true');
        }
    }
}
...
function confirmRate() {
    ...
    const marketOpen = localStorage.getItem('marketOpen');
    if (marketOpen === 'false') {
        // Market is closed - just open WhatsApp without pre-filled message
        const whatsappUrl = `https://wa.me/${whatsappNumber}`;
        window.open(whatsappUrl, '_blank');
        return;
    }
    
    // Market is open - send rate confirmation message
    const amount = parseFloat(amountInput.value) || 0;
    let currency = currencySelect.value;
    let denominationType = null;
    ...
    let formattedTotal;
    if (totalIDR % 1 === 0) {
        formattedTotal = totalIDR.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
    } else {
        formattedTotal = totalIDR.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
    }
